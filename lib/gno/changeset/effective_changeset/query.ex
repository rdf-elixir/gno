defmodule Gno.EffectiveChangeset.Query do
  @moduledoc false

  alias Gno.{Service, Changeset, EffectiveChangeset, NoEffectiveChanges}
  alias Gno.Store.SPARQL.Operation
  alias RDF.{Graph, Description}

  import Gno.QueryUtils

  def call(service, changeset)
  ...
  def call(service, changeset) do
    with {:ok, changeset} <- Changeset.new(changeset) do
      call(service, changeset)
    end
  end
